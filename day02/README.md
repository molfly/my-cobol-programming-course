...
Глагол ADD (который является новым в этом списке) увеличивает значение THE-NUMBER при отображении каждой строки. В операторах DISPLAY пробел используется для отделения номера строки от оператора.

## Layout and Punctuation
По мере того, как программы, над которыми вы работаете, становятся все больше, хорошей идеей будет начать уделять внимание макету и другим функциям COBOL, которые могут помочь сделать ваш код более читабельным.
Запятые можно использовать в COBOL для разделения элементов в списке:
```
DISPLAY THE-NUMBER, " ", THE-MESSAGE.
```
Есть аргументы за и против использования запятых. Что касается компилятора COBOL, запятые являются необязательными; компилятор полностью их игнорирует. Следовательно, единственное использование - улучшить читаемость. В списке переменных запятые помогают разделить элементы при чтении кода.

Предложение не обязательно должно начинаться и заканчиваться на одной строке. Пока он находится за пределами области A (столбцы с 8 по 11), предложение может занимать несколько строк. Листинг hello03.cbl, использует эту технику для четкого разделения каждого отображаемого значения. Листинг является версией jack02.cbl, которая распространяет предложение DISPLAY, чтобы уточнить, что отображается. Это скомпилирует и запустит идентично jack02.cbl, но его немного легче читать.

Помните, что когда вы читаете программы на языке COBOL, предложение продолжается до тех пор, пока не встретится точка, независимо от того, сколько строк оно занимает.

## Continuation Characters
Если буквенно-цифровое значение слишком длинное, чтобы поместиться в одной строке, его можно продолжить на следующей строке, используя символ продолжения. 

# Summary
Сегодня вы узнали основы об отделении данных COBOL, включая следующие:
* WORKING-STORAGE SECTION DATA DIVISION используется для создания пространства для переменных программы.
* Переменные могут быть названы с использованием заглавных букв от A до Z, цифр от 0 до 9 и дефиса (-). Дефис не может быть первым символом имени переменной. 
* Переменные могут быть названы с использованием заглавных букв от A до Z, цифр от 0 до 9 и дефиса (-). Дефис не может быть первым символом имени переменной. 
* Буквенно-цифровые переменные могут содержать печатные символы: от A до Z, от a до z, от 0 до 9, пробелы, символы и знаки пунктуации.
* Числовые переменные могут содержать числа (Numeric variables can hold numbers).
* Буквенно-цифровые значения должны быть заключены в двойные кавычки при перемещении в переменные. 
* Глагол MOVE перемещает буквенно-цифровое значение в алфавитно-цифровую переменную и дополняет переменную пробелами справа, если значение слишком короткое для заполнения переменной. 
* Глагол MOVE перемещает числовое значение в числовую переменную и добавляет нулевое значение слева, если значение слишком мало для заполнения переменной.
* Глагол DISPLAY может отображать более одного значения или переменной одновременно.
* Предложение COBOL может содержать запятые для пунктуации. Они не влияют на поведение конечной программы, но могут быть включены для улучшения читабельности.
* Предложение COBOL заканчивается точкой. Он может распространяться на несколько строк файла исходного кода, если он остается в области B, столбцы с 12 по 72.
* Символ продолжения может использоваться для продолжения литерала в одной или нескольких последующих строках.

# Q&A
Q: Есть ли другие ограничения на имена переменных?

A: На данный момент вы должны убедиться, что каждое имя переменной отличается. У вас есть до 30 символов для использования в качестве имени переменной, поэтому у вас не должно возникнуть проблем с поиском других имен.

Q: Когда вы должны использовать переменную, а когда вы должны использовать константу?

A: Большая часть работы в программировании выполняется с переменными. Вы можете использовать константу внутри `PROCEDURE DIVISION`, когда ее не нужно менять.

Константа никогда не изменится, но иногда проще использовать переменную, потому что она объясняет, что происходит. В следующем примере первая строка указывает, что сумма продаж умножается на константу .10 (10 процентов), но не дает информации о том, почему. Значение .10 является константой, поскольку его нельзя изменить без предварительного редактирования программы и ее повторной компиляции. Вторая версия указывает на то, что выполняется некоторая логика для расчета комиссии с продаж:
```
...
```
PICTURE определяет две вещи для переменной: размер переменной (количество байтов, используемых в памяти для значения).
30 Байт?
