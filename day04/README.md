...
> Делать - не делать: делайте осторожно тип `IF` и `IF-ELSE`. `IF` контролирует все операторы до следующего `ELSE` или до периода в конце предложения, если нет `ELSE`. `ELSE` контролирует все утверждения вплоть до периода в конце предложения.
НЕ используйте небрежный отступ для глаголов `IF` и `IF-ELSE`. Правильный отступ дает хорошее визуальное представление о том, какие части программы контролируются `IF`, а какие - `ELSE`.

> Последнее замечание в отступе `IF-ELSE` состоит в том, что COBOL, к сожалению, использует точку как терминатор предложения. Период почти невидим и может даже потеряться в распечатке с плохой лентой. Это еще одна причина, по которой исходный код должен быть максимально стандартизирован. Правильные IF-ELSE отступы - один из способов сделать ваш код легко читаемым.

# Саммари

Сегодняшний урок посвящен контролю потока программ путем тестирования условий с помощью оператора `IF`. Вы изучили эти основы: 

* Основным методом изменения потока программы является использование `IF` или `IF-ELSE` для принятия решения на основе значений переменных.
   
* Несколько операторов могут быть выполнены в `IF`.
   
* Условные операторы, используемые с тестом `IF`, следующие:

|Conditional operator    |Альтернативный оператор  |
|------------------------|-------------------------|
|IS EQUAL                |=                        |
|IS GREATER THAN         |>                        |
|IS LESS THAN            |<                        |
...
* Когда числовые значения сравниваются, они сравниваются как числа.
   
* Когда сравниваются буквенно-цифровые значения, они сравниваются на основе последовательности сортировки `ASCII`.
   
* Несколько условий могут быть проверены в `IF`, используя `AND` и `OR` для соединения двух или более сравнений.
 
* `ELSE` может использоваться для контроля операторов, которые должны выполняться, когда тест `IF` оценивается как ложный.

* Несколько операторов могут быть выполнены в `ELSE`.

* Операторы, контролируемые `IF`, выполняются, если условие, проверяемое `IF`, истинно.

* Операторы, контролируемые `ELSE`, выполняются, если условие, проверяемое соответствующим `IF`, ложно.